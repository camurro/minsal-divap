...
public class EstimacionFlujoCajaService {
	...
	public void calcularPropuesta(Integer idProgramaAno,
			Boolean iniciarFlujoCaja) {
		if (iniciarFlujoCaja) {
			ProgramaAno programaAno = programasDAO
					.getProgramaAnoByID(idProgramaAno);
			ProgramaAno programaAnoAnterior = programasDAO
					.getProgramaAnoByIDProgramaAno(programaAno.getPrograma()
							.getId(), (getAnoCurso() - 1));
			if (programaAnoAnterior != null) {
				for (MarcoPresupuestario marcoPresupuestarioAnoAnterior : programaAnoAnterior
						.getMarcosPresupuestarios()) {
					MarcoPresupuestario marcoPresupuestario = new MarcoPresupuestario();
					marcoPresupuestario.setIdProgramaAno(programaAno);
					marcoPresupuestario
							.setMarcoInicial((int) (marcoPresupuestarioAnoAnterior
									.getMarcoModificado() * programaAno
									.getAno().getInflactorMarcoPresupuestario()));
					marcoPresupuestario
							.setServicioSalud(marcoPresupuestarioAnoAnterior
									.getServicioSalud());
					cajaDAO.save(marcoPresupuestario);
					List<Caja> flujosCaja = cajaDAO
							.getByIdProgramaAnoIdServicio(programaAnoAnterior
									.getIdProgramaAno(),
									marcoPresupuestarioAnoAnterior
					...
					if (flujosCaja != null && flujosCaja.size() > 0) {
						...
						for (Caja cajaAnoAnterior : flujosCaja) {
							...
							cajaActual
									.setMarcoPresupuestario(marcoPresupuestario);
							cajaActual.setIdComponente(cajaAnoAnterior
									.getIdComponente());
							cajaActual.setIdSubtitulo(cajaAnoAnterior
									.getIdSubtitulo());
							int montoActual = (int) (cajaAnoAnterior.getMonto() * programaAno
									.getAno().getInflactorMarcoPresupuestario());
							cajaActual.setMonto(montoActual);
							cajaDAO.save(cajaActual);
							if (cajaAnoAnterior.getCajaMontos() != null
									&& cajaAnoAnterior.getCajaMontos().size() > 0) {
								for (CajaMonto cajaMontoAnterior : cajaAnoAnterior
										.getCajaMontos()) {
									CajaMonto cajaMontoActual = new CajaMonto();
									Mes mes = mesDAO
											.getMesPorID(cajaMontoAnterior
													.getMes().getIdMes());
									CajaMontoPK cajaMontoPK = new CajaMontoPK(
											cajaActual.getId(),
											cajaMontoAnterior.getMes()
													.getIdMes());
									...
									if (cajaMontoAnterior.getMonto() != null) {
										...
										int porcentaje = cajaMontoAnterior
												.getMonto()
												.getPorcentajeMonto();
										int monto = (int) (montoActual * (porcentaje / 100.0));
										montoMesActual
												.setPorcentajeMonto(porcentaje);
										montoMesActual.setMonto(monto);
										montoMesActual
												.setFechaMonto(new Date());
										cajaDAO.save(montoMesActual);
										cajaMontoActual
												.setMonto(montoMesActual);
									}
				...
				programaAno.setEstadoFlujoCaja(new EstadoPrograma(
						EstadosProgramas.ENREVISION.getId()));
			}
		} else {
			...
			DistribucionInicialPercapita distribucionInicialPercapita = distribucionInicialPercapitaDAO
					.findLast();
			List<Object[]> percapitaServicios = antecedentesComunaDAO
					.groupPercapitaServicioByDistribucionInicialPercapita(distribucionInicialPercapita
			...
			if (percapitaServicios != null && percapitaServicios.size() > 0) {
				for (Object[] percapitaServicio : percapitaServicios) {
					Integer idServicio = ((Number) (percapitaServicio[0]))
							.intValue();
					Long percapitaAno = ((Number) (percapitaServicio[1]))
							.longValue();
					MarcoPresupuestario marcoPresupuestario = cajaDAO
							.getMarcoPresupuestarioByProgramaAnoServicio(
									idProgramaAno, idServicio);
					if (percapitaAno != null) {
						marcoPresupuestario.setMarcoModificado(percapitaAno
								.intValue());
					} else {
						marcoPresupuestario.setMarcoModificado(0);
					}
					List<Caja> cajas = cajaDAO.getByIdProgramaAnoIdServicio(
							idProgramaAno, idServicio);
					if (cajas != null && cajas.size() > 0) {
						// Obtenemos el programa actual.
						for (Caja cajaActual : cajas) {
							Integer idComponente = cajaActual.getIdComponente()
									.getId();
							Integer idSubtitulo = cajaActual.getIdSubtitulo()
									.getIdTipoSubtitulo();
							Integer totalComponenteSubtitulo = 0;
							List<ProgramaMunicipalCoreComponente> programaMunicipalesCore = programasDAO
									.getProgramaMunicipales(idProgramaAno,
											idComponente, idSubtitulo);
							if (programaMunicipalesCore != null
									&& programaMunicipalesCore.size() > 0) {
								for (ProgramaMunicipalCoreComponente programaMunicipalCoreComponente : programaMunicipalesCore) {
									totalComponenteSubtitulo += programaMunicipalCoreComponente
											.getMonto();
								}
							}
							List<ProgramaServicioCoreComponente> programaServiciosCore = programasDAO
									.getProgramaServicios(idProgramaAno,
											idComponente, idSubtitulo);
							if (programaServiciosCore != null
									&& programaServiciosCore.size() > 0) {
								for (ProgramaServicioCoreComponente programaServicioCoreComponente : programaServiciosCore) {
									totalComponenteSubtitulo += programaServicioCoreComponente
											.getMonto();
								}
							}
							cajaActual.setMonto(totalComponenteSubtitulo);
							marcoPresupuestario
									.setMarcoModificado(marcoPresupuestario
											.getMarcoModificado()
											+ totalComponenteSubtitulo);
						}
						for (Caja cajaActual : cajas) {
							if (cajaActual.getCajaMontos() != null
									&& cajaActual.getCajaMontos().size() > 0) {
								for (CajaMonto cajaMonto : cajaActual
										.getCajaMontos()) {
									int porcentaje = cajaMonto.getMonto()
											.getPorcentajeMonto();
	...
	public Integer createSeguimiento(Integer idLineaProgramatica,
		...
		estimacionFlujoCajaDAO.createSeguimiento(idLineaProgramatica,
				seguimiento);
		return 1;// distribucionInicialPercapitaDAO.createSeguimiento(idDistribucionInicialPercapita,
		// seguimiento);
	}

	public List<SeguimientoVO> getBitacora(Integer idPorgramaAno,
			TareasSeguimiento tareaSeguimiento) {
		return seguimientoService.getBitacoraEstimacionFlujoCaja(idPorgramaAno,
				tareaSeguimiento);
	}
	...
	public Integer generarPlanillaPropuesta(Integer idLineaProgramatica) {
		...
		System.out.println("idLineaProgramatica --> "+idLineaProgramatica);
		System.out.println("Subtitulo.SUBTITULO21 --> "+Subtitulo.SUBTITULO21);
		
		
		
		List<CajaMesVO> cajaMeses = new ArrayList<CajaMesVO>();
		List<FlujoCajaVO> flujoCajaSub21 = getFlujoCajaServicios(
				idLineaProgramatica, Subtitulo.SUBTITULO21);
		List<FlujoCajaVO> flujoCajaSub22 = getFlujoCajaServicios(
				idLineaProgramatica, Subtitulo.SUBTITULO22);

		System.out.println("flujoCajaSub21.size() ---> "
				+ flujoCajaSub21.size());
		for (int i = 0; i < flujoCajaSub21.size(); i++) {
			System.out.println("flujoCajaSub21.get(i).getMontoSubtitulo() --> "
					+ flujoCajaSub21.get(i).getMontoSubtitulo());
		}

		List<CajaMesVO> cajaMesesResumen = new ArrayList<CajaMesVO>();
		List<FlujoCajaVO> flujoCajaResumen = new ArrayList<FlujoCajaVO>();

		List<FlujoCajaVO> flujoCajaSub24 = getFlujoCajaServicios(
				idLineaProgramatica, Subtitulo.SUBTITULO24);
		List<FlujoCajaVO> flujoCajaSub29 = getFlujoCajaServicios(
				idLineaProgramatica, Subtitulo.SUBTITULO29);
		/*
		 * for(int i=0;i<12;i++){
		 * 
		 ...
		 * 
		 * if(i == 0){ cajaMes.setMes("enero"); cajaMes.setMonto((long)montoMes
		 * - 5000000); cajaMeses.add(cajaMes); } else if(i == 1){
		 * cajaMes.setMes("febrero"); cajaMes.setMonto((long)montoMes +
		 * 30000000); cajaMeses.add(cajaMes); }else if(i == 2){
		 * cajaMes.setMes("marzo"); cajaMes.setMonto((long)montoMes);
		 * cajaMeses.add(cajaMes); }else if(i == 3){ cajaMes.setMes("abril");
		 * cajaMes.setMonto((long)montoMes + 4000000); cajaMeses.add(cajaMes);
		 * }else if(i == 4){ cajaMes.setMes("mayo");
		 * cajaMes.setMonto((long)montoMes + 12000000); cajaMeses.add(cajaMes);
		 * }else if(i == 5){ cajaMes.setMes("junio");
		 * cajaMes.setMonto((long)montoMes); cajaMeses.add(cajaMes); }else if(i
		 * == 6){ cajaMes.setMes("julio"); cajaMes.setMonto((long)montoMes -
		 * 2000000); cajaMeses.add(cajaMes); }else if(i == 7){
		 * cajaMes.setMes("agosto"); cajaMes.setMonto((long)montoMes + 3500000);
		 * cajaMeses.add(cajaMes); }else if(i == 8){
		 * cajaMes.setMes("septiembre"); cajaMes.setMonto((long)montoMes -
		 * 3500000); cajaMeses.add(cajaMes); }else if(i == 9){
		 * cajaMes.setMes("octubre"); cajaMes.setMonto((long)montoMes -
		 * 2000000); cajaMeses.add(cajaMes); }else if(i == 10){
		 * cajaMes.setMes("noviembre"); cajaMes.setMonto((long)montoMes -
		 * 25000000); cajaMeses.add(cajaMes); }else if(i == 11){
		 * cajaMes.setMes("diciembre"); cajaMes.setMonto((long)montoMes +
		 * 1000000 ); cajaMeses.add(cajaMes); } }
		 * 
		 * for (int i=0; i<5; i++){ FlujoCajaVO flujo1 = new FlujoCajaVO();
		 * if(i==1){ flujo1.setServicio("Santiago");
		 * flujo1.setSubtitulo(cajaMeses); flujoCajaSub21.add(flujo1);
		 * flujoCajaSub22.add(flujo1);
		 * 
		 * //al final setear montoSubtitulo } if(i==2){
		 * flujo1.setServicio("Curico"); flujo1.setSubtitulo(cajaMeses);
		 * flujoCajaSub21.add(flujo1); flujoCajaSub22.add(flujo1); //al final
		 * setear montoSubtitulo } if(i==3){ flujo1.setServicio("Talca");
		 * flujo1.setSubtitulo(cajaMeses); flujoCajaSub21.add(flujo1);
		 * flujoCajaSub22.add(flujo1); //al final setear montoSubtitulo }
		 * if(i==4){ flujo1.setServicio("Concepcion");
		 * flujo1.setSubtitulo(cajaMeses); flujoCajaSub21.add(flujo1);
		 * flujoCajaSub22.add(flujo1); //al final setear montoSubtitulo }
		 * if(i==5){ flujo1.setServicio("Punta Arenas");
		 * flujo1.setSubtitulo(cajaMeses); flujoCajaSub21.add(flujo1);
		 * flujoCajaSub22.add(flujo1); //al final setear montoSubtitulo }
		 * 
		 * }
		...
		// header.add(new
		// CellExcelVO("TOTAL CHILE CRECE CONTIGO SUBT. XX AÑO 2014", 1, 2));
		...
		headerTotales.add(new CellExcelVO(
				"TOTAL MENSUAL APS SUBT. 24, 21, 22 Y 29 AÑO 2014 ($)", 1, 2));
		...
		EstimacionFlujoCajaSubtituloSheetExcel estimacionFlujoCajaSubtituloSheetExcel = new EstimacionFlujoCajaSubtituloSheetExcel(
				header, subHeader, items);

		EstimacionFlujoCajaSubtituloSheetExcel estimacionFlujoCajaResumenSheetExcel = new EstimacionFlujoCajaSubtituloSheetExcel(
				headerTotales, subHeaderTotales, totalItems);
		...
		if (flujoCajaSub21 != null && !flujoCajaSub21.isEmpty()) {
			...
			for (int i = 0; i < totalItems.size(); i++) {
				...
				totalfebrero += totalItems.get(i).getSubtitulo().get(1)
						.getMonto();
				totalmarzo += totalItems.get(i).getSubtitulo().get(2)
						.getMonto();
				totalabril += totalItems.get(i).getSubtitulo().get(3)
						.getMonto();
				totalmayo += totalItems.get(i).getSubtitulo().get(4).getMonto();
				totaljunio += totalItems.get(i).getSubtitulo().get(5)
						.getMonto();
				totaljulio += totalItems.get(i).getSubtitulo().get(6)
						.getMonto();
				totalagosto += totalItems.get(i).getSubtitulo().get(7)
						.getMonto();
				totalseptiembre += totalItems.get(i).getSubtitulo().get(8)
						.getMonto();
				totaloctubre += totalItems.get(i).getSubtitulo().get(9)
						.getMonto();
				totalnoviembre += totalItems.get(i).getSubtitulo().get(10)
						.getMonto();
				totaldiciembre += totalItems.get(i).getSubtitulo().get(11)
						.getMonto();
			}

			generadorExcel.addSheet(estimacionFlujoCajaSubtituloSheetExcel,
					"21");
		}
		if (flujoCajaSub22 != null && !flujoCajaSub22.isEmpty()) {
			header.add(new CellExcelVO(
					"TOTAL CHILE CRECE CONTIGO SUBT. 22 AÑO 2014", 1, 2));
		...
		if (flujoCajaSub24 != null && !flujoCajaSub24.isEmpty()) {
			header.add(new CellExcelVO(
					"TOTAL CHILE CRECE CONTIGO SUBT. 24 AÑO 2014", 1, 2));
		...
		if (flujoCajaSub29 != null && !flujoCajaSub29.isEmpty()) {
			header.add(new CellExcelVO(
					"TOTAL CHILE CRECE CONTIGO SUBT. 29 AÑO 2014", 1, 2));
		...
		// generadorExcel.addSheet(estimacionFlujoCajaResumenSheetExcel,
		// "Resumen");
	...
	private List<FlujoCajaVO> getFlujoCajaServicios(
		...
		List<Caja> cajas = estimacionFlujoCajaDAO.getFlujoCajaServicios(
				idLineaProgramatica, subtitulo);
		if (cajas != null && cajas.size() > 0) {
			for (Caja caja : cajas) {
	...
	public Integer eliminarOrdinarioFonasa(Integer idLineaProgramatica) {
		...
		ReferenciaDocumentoSummaryVO referenciaDocumentoSummary = getLastDocumentSummaryEstimacionFlujoCajaType(
				idLineaProgramatica,
				TipoDocumentosProcesos.PLANTILLABORRADORORDINARIOPROGRAMACIONCAJA);
		...
		String keyUltimo = ((referenciaDocumentoSummary.getNodeRef() == null) ? referenciaDocumentoSummary
				.getPath() : referenciaDocumentoSummary.getNodeRef().replace(
	...
	public void moveToAlfresco(Integer idLineaProgramatica,
			...
			TipoDocumentosProcesos tipoDocumento, boolean versionFinal) {
		...
		if (referenciaDocumentoSummary != null) {
			...
			documentService.updateDocumentTemplate(
					...
					response.getFileName(), contenType);
			// DistribucionInicialPercapita distribucionInicialPercapita =
			// distribucionInicialPercapitaDAO
			// .findById(idDistribucionInicialPercapita);
			ProgramaAno programaAno = programasDAO
					.getProgramaAnoByID(idLineaProgramatica);
			documentService
					.createDocumentOrdinarioProgramacionEstimacionFlujoCaja(
							programaAno, tipoDocumento, referenciaDocumentoId,
							versionFinal);
	...
	public ReferenciaDocumentoSummaryVO getLastDocumentSummaryEstimacionFlujoCajaType(
		...
		return documentService
				.getLastDocumentoSummaryByEstimacionFlujoCajaType(
						idProgramaAno, tipoDocumento);

	}

	// Obtiene el id del programa para el a�o siguiente para hacer la estimacion
	// del flujo de caja.
	public Integer obtenerIdProgramaAno(Integer id) {
		...
		if (programaAnoSiguiente == null) {
			...
			AnoEnCurso anoCurso = programasDAO
					.getAnoEnCursoById(programaAnoActual.getAno().getAno() + 1);
			if (anoCurso == null) {
			...
			EstadoPrograma estadoProgramaCurso = new EstadoPrograma(
					EstadosProgramas.ENCURSO.getId());
			EstadoPrograma estadoProgramaIniciar = new EstadoPrograma(
					EstadosProgramas.SININICIAR.getId());
			...
			idProgramaAnoNuevo = programasDAO.saveProgramaAno(
					programaAnoActual, true);
			// estadoPrograma );
		} else {
	...
	public Integer enviarOrdinarioFONASA(Integer idLineaProgramatica) {
		...
		ReferenciaDocumentoSummaryVO referenciaDocumentoSummary = getLastDocumentSummaryEstimacionFlujoCajaType(
				idLineaProgramatica,
				TipoDocumentosProcesos.PLANTILLABORRADORORDINARIOPROGRAMACIONCAJA);
		DocumentoVO documentDecretoAporteEstatalVO = documentService
				.getDocument(referenciaDocumentoSummary.getId());

		String fileNameDecretoAporteEstatal = tmpDirDoc + File.separator
				+ documentDecretoAporteEstatalVO.getName();

		GeneradorWord generadorWordDecretoAporteEstatal = new GeneradorWord(
				fileNameDecretoAporteEstatal);
		try {
			generadorWordDecretoAporteEstatal.saveContent(
					documentDecretoAporteEstatalVO.getContent(),
					XWPFDocument.class);
			...
			adjunto.setUrl((new File(fileNameDecretoAporteEstatal)).toURI()
					.toURL());
			adjuntos.add(adjunto);
			emailService
					.sendMail(
							"f.pachecogajardo@gmail.com",
							"Borrador Decreto Aporte Estatal",
							"Estimado "
									+ "usuario prueba"
									+ ": <br /> <p> Se Adjunta Borrador Decreto Aporte Estatal</p>",
							adjuntos);
			System.out
					.println("enviarDecretoAporteEstatal se ejecuto correctamente");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return 1;
	...
	public List<SubtituloFlujoCajaVO> getMonitoreoByProgramaAnoComponenteSubtitulo(
			Integer IdProgramaAno, Integer idComponente, Subtitulo subtitulo) {
		List<SubtituloFlujoCajaVO> subtitulosFlujosCaja = new ArrayList<SubtituloFlujoCajaVO>();
		List<Caja> flujosCaja = cajaDAO
				.getMonitoreoByProgramaAnoComponenteSubtitulo(IdProgramaAno,
						idComponente, subtitulo);
		if (flujosCaja != null && flujosCaja.size() > 0) {
			for (Caja caja : flujosCaja) {
				subtitulosFlujosCaja.add(new SubtituloFlujoCajaMapper()
						.getBasic(caja, Integer.parseInt(getMesCurso(true))));
			}
		}
		return subtitulosFlujosCaja;
	}

	public List<SubtituloFlujoCajaVO> getConvenioRemesaByProgramaAnoComponenteSubtitulo(
			Integer idProgramaAno, Integer idComponente, Subtitulo subtitulo) {
		List<SubtituloFlujoCajaVO> subtitulosFlujosCaja = new ArrayList<SubtituloFlujoCajaVO>();
		List<Caja> flujosCaja = cajaDAO
				.getConvenioRemesaByProgramaAnoComponenteSubtitulo(
						idProgramaAno, idComponente, subtitulo);
		if (flujosCaja != null && flujosCaja.size() > 0) {
			for (Caja caja : flujosCaja) {
				subtitulosFlujosCaja
						.add(new ConvenioRemesaSubtituloFlujoCajaMapper()
								.getBasic(caja,
	...
	public Integer obtenerIdProgramaAnoSiguiente(Integer idProgramaAno,
			Integer anoSiguiente) {
		ProgramaAno programaAnoActual = programasDAO
				.getProgramaAnoByID(idProgramaAno);
		ProgramaAno programaAnoSiguiente = programasDAO
				.getProgramaAnoSiguiente(programaAnoActual.getPrograma()
						.getId(), anoSiguiente);
		if (programaAnoSiguiente == null) {
			// crear
			programaAnoSiguiente = new ProgramaAno();
			AnoEnCurso anoSiguienteDTO = programasDAO
					.getAnoEnCursoById(anoSiguiente);
			if (anoSiguienteDTO == null) {
				AnoEnCurso anoActualDTO = programasDAO
						.getAnoEnCursoById(anoSiguiente - 1);
				anoSiguienteDTO = new AnoEnCurso();
				anoSiguienteDTO.setAno(anoSiguiente);
				anoSiguienteDTO.setMontoPercapitalBasal(anoActualDTO
						.getMontoPercapitalBasal());
				anoSiguienteDTO.setAsignacionAdultoMayor(anoActualDTO
						.getAsignacionAdultoMayor());
				anoSiguienteDTO.setInflactor(anoActualDTO.getInflactor());
				anoSiguienteDTO.setInflactorMarcoPresupuestario(anoActualDTO
						.getInflactorMarcoPresupuestario());
	...
	public List<SubtituloFlujoCajaVO> getPercapitaByProgramaAno(Integer idProgramaAno) {
		...
		Integer id_disperc = distribucionInicialPercapitaDAO.findLast().getIdDistribucionInicialPercapita();
		...
		if(antecendentesComunasCalculado != null && antecendentesComunasCalculado.size()>0){
			...
			for(AntecendentesComunaCalculado antecendentesComunaCalculado : antecendentesComunasCalculado){
				...
				if(percapitaServicio.containsKey(antecendentesComunaCalculado.getAntecedentesComuna().getIdComuna().getServicioSalud().getId())){
					...
					System.out.println("hasmap, key -> "+antecendentesComunaCalculado.getAntecedentesComuna().getIdComuna().getServicioSalud().getId()+"  valor --> "+valor);
				}else{
					...
					System.out.println("hasmap, key -> "+antecendentesComunaCalculado.getAntecedentesComuna().getIdComuna().getServicioSalud().getId()+"  valor --> "+antecendentesComunaCalculado.getPercapitaAno());
				}
			}
		}
		 ...
		 for (Integer key : percapitaServicio.keySet()) {
			 ...
			 System.out.println("valor key ---> "+valor);
			 System.out.println("valor del key --> "+key);
			 SubtituloFlujoCajaVO subtituloFlujoCajaVO = new SubtituloFlujoCajaVO();
			 ServiciosVO servicioSalud = servicioSaludService.getServicioSaludPorID(key);
			 System.out.println("servicioSalud--->>"+servicioSalud);
			 if(servicioSalud != null)
			 subtituloFlujoCajaVO.setServicio(servicioSalud.getNombre_servicio()); //setear nombre servicio rescatado con el id  key
			 ...
			 for(int i=0; i<12; i++){
				 ...
				 System.out.println("monto mensual --> "+caja.getMontoMes());
			 ...
			 System.out.println("nombre del servicio ---> "+subtituloFlujoCajaVO.getServicio());
